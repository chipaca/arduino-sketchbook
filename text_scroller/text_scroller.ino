...
char* message = "  Hola Fede!!! ;-)   0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ!\"#$%&\\'()*+,-./:;<=>?@[\\]^_`{|}~  ";

// from X11 misc 5x8 pcf file. via:
// pcf2bdf /usr/share/fonts/X11/misc/5x8.pcf.gz | perl -00 -wne 'if (/^STARTCHAR (\w+).*BITMAP.(.*)ENDCHAR/ms) {$n=$1; $b=$2; print("  {", join(", ", map("0x$_", $b =~ /(\w+)/g)), "}, // $n\n")}'
byte ascii[][8] = {
...
void setup() {
  message[strlen(message)-2] = message[0];
  message[strlen(message)-1] = message[1];
  int i;
  for (i=2; i<10; i++) {
    pinMode(i, OUTPUT);
    digitalWrite(i, LOW);
...
void loop() {
  ...
  if (!tick) {
    for (i=0; i<8; i++) {
      msg[7-i] = ((ascii[message[pos]-' '][i] << off) & 255) + (ascii[message[pos+1]-' '][i] >> (5-off)) + (ascii[message[pos+2]-' '][i] >> (10-off));
    }
    off++;
    if (off > 4) {
      pos++;
      off=0;
      if (pos >= strlen(message) - 2) {
        pos = 0;
      }
    }
  }
